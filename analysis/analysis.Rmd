...
if(file.exists("models/data_D_scores_with_bootstrapped_CIs.rds")) {
  ...
  data_bootstrapped_CIs <- 
    ...
    mutate(sig = ifelse((D_ci_lower < 0 & D_ci_upper < 0) | (D_ci_lower > 0 & D_ci_upper > 0), TRUE, FALSE),
           D_ci_width = D_ci_upper - D_ci_lower) %>%
    round_df(3)
  
  data_D_scores_with_bootstrapped_CIs <- data_bootstrapped_CIs %>%
    ...
    select(session_id, domain, D, D_ci_lower, D_ci_upper, sig, D_ci_width)
  ...
  # save to disk
  write_rds(data_D_scores_with_bootstrapped_CIs, "models/data_D_scores_with_bootstrapped_CIs.rds")
...
ggplot(data_D_scores_with_bootstrapped_CIs) +
  ...
  geom_linerange(aes(x = session_id, ymin = D_ci_lower, ymax = D_ci_upper, color = Effect), 
                 alpha = 0.3) + 
